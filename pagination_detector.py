...
def detect_pagination_elements(markdown_content: str, base_url: str) -> List[str]:
    ...
    try:
        ...
        Rules:
        1. Look for any of these pagination patterns:
           - Links containing 'page-' or 'page='
           - Links with '/page/2', '/page/3', etc.
           - Links ending with 'page-2.html', 'page-3.html', etc.
           - Navigation links with text like 'Next', '»', '2', '3', etc.
        ...
        # Process URLs with expanded patterns
        urls = []
        pagination_patterns = [
            r'page-\d+\.html',  # For books.toscrape.com
            r'page_num=\d+',    # For scrapethissite.com
            r'/page/\d+/?',     # For WordPress-style
            r'page=\d+',        # For query parameter style
            r'/p/\d+',          # Alternative format
            r'[?&]p=\d+'        # Query parameter format
        ]
        ...
        for line in response_text.split('\n'):
            ...
            # Check if line matches any pagination pattern
            if any(re.search(pattern, line) for pattern in pagination_patterns):
        ...
        logging.info(f"Detected {len(unique_urls)} pagination URLs")
        if unique_urls:
            logging.info(f"Found pagination URLs: {unique_urls}")
